 ...
 * @author Basil Hosmer
 */
public final class Plus extends IntrinsicLambda
{
    public static final String NAME = "plus";

    private static final TypeParam T = new TypeParam("T");

    public static final Type TYPE = Types.fun(Types.tup(T, T), T);
    ...
    public static Object invoke(final Object left, final Object right)
    {
        ...
        {
            return LPlus.invoke((ListValue)left, (ListValue)right);
        }
        else if (left instanceof MapValue)
        {
            return MPlus.invoke((MapValue)left, (MapValue)right);
        }
        ...
        else
        {
            Session.error("internal error: plus() undefined over {0}, {1} ({2}, {3})",
                ToStr.invoke(left), ToStr.invoke(right),
